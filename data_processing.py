...
def process_embed_data(
    ...
    for embed in embeds:
        ...
        is_chat = any(":inbox_tray:" in l or ":outbox_tray:" in l for l in lines)
        ...
        for line in lines:
            if ":outbox_tray:" in line:
                ...
                if player: players_in_session.add(player)
                for admin in admins_in_session:
                    if admin != player: admin_stats[admin]["mentions"] += 1
                for admin in admin_only_admins_in_session:
                    if admin != player: admin_stats[admin]["admin_only_mentions"] += 1
        ...
        if is_chat:
            ...
            for admin in admins_in_session:
                ...
                if has_ahelp and admin not in players_in_session:
                    ...
                    if message_datetime: daily_ahelps[admin] += 1
            for admin in admin_only_admins_in_session:
                admin_stats[admin]["admin_only_sessions"] += 1
                if has_ahelp and admin not in players_in_session:
                    admin_stats[admin]["admin_only_ahelps"] += 1
                    if message_datetime: daily_admin_only_ahelps[admin] += 1

        if has_ahelp:
            total_ahelps_count += 1
            if admin_responded: processed_ahelps_count += 1

    return (dict(admin_stats), chat_count, dict(daily_ahelps),
            dict(daily_admin_only_ahelps), total_ahelps_count, processed_ahelps_count)
...
def load_json_file(file_path: str) -> Optional[Any]:
    ...
    try:
        with open(file_path, 'r', encoding='utf-8') as f:
            data = json.load(f)
            logging.debug(f"Успешно загружен JSON файл: {file_path}")
            return data
...
def analyze_ahelp_data(data: Any, server_name: str, start_date: Optional[datetime] = None,
    ...
    if not isinstance(data, list):
        ...
        return {"admin_stats": {}, "chat_count": 0, "daily_ahelps": {}, "hourly_ahelps": {},
                "daily_admin_only_ahelps": {}}

    logging.debug(f"Анализ {len(data)} сообщений с сервера {server_name}")
    for message in data:
        message_timestamp = message.get('created_at', '')
        msg_dt = parse_message_time(message_timestamp)
        if not msg_dt:
            logging.warning(f"Неверная или отсутствующая дата 'created_at' в сообщении ID {message.get('id')}; пропуск.")
            continue
    ...
    return {
        "admin_stats": dict(admin_stats), "chat_count": total_chat_count,
        ...
        "hourly_ahelps": {d: {h: vals.copy() for h, vals in hours.items()} for d, hours in hourly_ahelps.items()}
    }
...
def merge_duplicate_admins(admin_stats: Dict[str, AdminStats]) -> Dict[str, AdminStats]:
    ...
    for admin, stats in admin_stats.items():
        ...
        if stats["role"] != "Unknown":
            merged_stats[normalized_admin]["role"] = normalize_admin_string(stats["role"])
        merged_stats[normalized_admin]["sessions"] += stats["sessions"]
    return dict(merged_stats)


def fill_missing_roles(
        merged_admin_stats: Dict[str, AdminStats], servers_stats: Dict[str, ServerStats]
) -> None:
        ...
        if stats["role"] == "Unknown":
            for server_stats in servers_stats.values():
                ...
                if admin in server_admin_stats and server_admin_stats[admin]["role"] != "Unknown":
                    ...
                    logging.debug(f"Заполнена отсутствующая роль для админа '{admin}' из статистики сервера.")
                    break